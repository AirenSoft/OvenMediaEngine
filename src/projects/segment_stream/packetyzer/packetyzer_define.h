...
enum class PacketyzerStreamType
{
	Common,		// Video + Audio
	VideoOnly,
	AudioOnly,
...
{
public :
	SegmentData(int sequence_number_, std::string &file_name_, uint64_t duration_, uint64_t timestamp_, int data_size_, uint8_t *data_)
	{
	...
	SegmentData(int sequence_number_, std::string &file_name_, uint64_t duration_, uint64_t timestamp_, std::shared_ptr<std::vector<uint8_t>> &data_)
	{
		sequence_number = sequence_number_;
		file_name		= file_name_;
		create_time		= time(nullptr);
