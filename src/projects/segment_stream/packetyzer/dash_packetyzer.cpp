...
bool DashPacketyzer::AppendVideoFrame(std::shared_ptr<PacketyzerFrameData> &frame_data)
{
    ...
    if (frame_data->type == PacketyzerFrameType::VideoIFrame && !_video_frame_datas.empty())
    {
        ...
        if ((frame_data->timestamp - _video_frame_datas[0]->timestamp) >
            ((((double) _segment_duration) * 0.87) * _media_info.video_timescale)) // I-Frame 관련 threshold 87% 한도 내에서 처리
        {
            // Video Segment Write
            ...
            {
                AudioSegmentWrite(ConvertTimeScale(frame_data->timestamp, _media_info.video_timescale,
                                                   _media_info.audio_timescale));
            }
...
bool DashPacketyzer::VideoSegmentWrite(uint64_t last_timestamp)
{
    ...
    // Fragment 쓰기
    auto fragment_writer = std::make_unique<M4sFragmentWriter>(M4sMediaType::VideoMediaType, 4096,
                                                               _video_sequence_number, VIDEO_TRACK_ID, start_timestamp,
                                                               sample_datas);
    ...
    // m4s 데이터 저장
    SetSegmentData(SegmentDataType::Mp4Video, _sequence_number, file_name.str(), last_timestamp - start_timestamp,
                   start_timestamp, data_stream, true);
...
bool DashPacketyzer::AudioSegmentWrite(uint64_t last_timestamp)
{
    ...
    // Fragment 쓰기
    auto fragment_writer = std::make_unique<M4sFragmentWriter>(M4sMediaType::AudioMediaType, 4096,
                                                               _audio_sequence_number, AUDIO_TRACK_ID, start_timestamp,
                                                               sample_datas);
    ...
    // m4s 데이터 저장
    SetSegmentData(SegmentDataType::Mp4Audio, _sequence_number, file_name.str(), end_timestamp - start_timestamp,
                   start_timestamp, data_stream, true);
...
bool DashPacketyzer::UpdatePlayList(bool video_update)
{
    ...
    for (int index = std::max((int) (_video_segment_indexer.size() - _segment_count), 0);
        ...
        // Timeline Setting
        if (!video_first_segment_check)
            video_segment_urls << "\t\t\t\t" << "<S t=\"" << item->second->timestamp << "\" d=\""
        ...
        // 최소 duration
        if (video_min_duration == 0 || video_min_duration > item->second->duration)
            video_min_duration = item->second->duration;
    }
         ...
         index < _audio_segment_indexer.size(); index++)
    {
        ...
        if (audio_first_segment_check) audio_segment_urls << "\t\t\t\t<S d=\"" << item->second->duration << "\"/>\n";
            ...
            audio_segment_urls << "\t\t\t\t<S t=\"" << item->second->timestamp << "\" d=\"" << item->second->duration
                               << "\"/>\n";
        ...
        // 최소 duration
        if (audio_min_duration == 0 || audio_min_duration > item->second->duration)
            audio_min_duration = item->second->duration;
    }
    ...
    uint32_t suggested_presentation_delay = 0;
    ...
    if (_stream_type == PacketyzerStreamType::AudioOnly)
    {
        if (_media_info.audio_timescale != 0)
        {
            ...
            suggested_presentation_delay = (uint32_t) audio_total_duration / _media_info.audio_timescale / 2;
        }
    ...
    else
    {
        if (_media_info.video_timescale != 0)
        {
            ...
            suggested_presentation_delay = (uint32_t) video_total_duration / _media_info.video_timescale / 2;
        }
    }
    ...
    play_list << "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"
              ...
              << "    suggestedPresentationDelay=\"PT" << suggested_presentation_delay << "S\"\n"
    ...
    {
        play_list
                << "\t<AdaptationSet id=\"1\" group=\"2\" mimeType=\"audio/mp4\" lang=\"und\" segmentAlignment=\"true\" startWithSAP=\"1\" subsegmentAlignment=\"true\" subsegmentStartsWithSAP=\"1\">\n"
