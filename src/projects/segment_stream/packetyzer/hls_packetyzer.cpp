...
HlsPacketyzer::HlsPacketyzer(std::string			&segment_prefix,
							...
							Packetyzer(PacketyzerType::Hls, segment_prefix, stream_type, segment_count, (uint32_t)segment_duration, media_info)
{
...
bool HlsPacketyzer::AppendAudioFrame(std::shared_ptr<PacketyzerFrameData>  &frame_data)
{
	...
	uint64_t timestamp = (frame_data->timescale == _media_info.audio_timescale) ? frame_data->timestamp : ConvertTimeScale(frame_data->timestamp, frame_data->timescale, _media_info.audio_timescale);
...
bool HlsPacketyzer::UpdatePlayList( )
{
	...
	for(int index  = std::max(0, (int)(_segment_indexer.size() - _segment_count)); index < _segment_indexer.size(); index++)
	{
		...
		if(item != _segment_datas.end())
		{
			...
			m3u8_play_list 	<< "#EXTINF:" << std::fixed << std::setprecision(3) << segment_data->duration/PACKTYZER_DEFAULT_TIMESCALE << ",\r\n"
							<< file_name << "\r\n";
