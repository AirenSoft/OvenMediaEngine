...
bool OvenCodecImplAvcodecEncJpeg::Configure(std::shared_ptr<TranscodeContext> context)
{
...
void OvenCodecImplAvcodecEncJpeg::Stop()
{
	_kill_flag = true;
	...
	while(!_kill_flag)
	{
		...
		while(true)
		{
			...
			else
			{
				...
				packet_buffer->SetBitstreamFormat(common::BitstreamFormat::JPEG);
