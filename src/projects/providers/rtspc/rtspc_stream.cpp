...
namespace pvd
{
	...
	bool RtspcStream::ConnectOrigin()
	{
	...
	bool RtspcStream::ReceiveDescribe(uint32_t request_id)
	{
	...
	bool RtspcStream::ReceivePlay(uint32_t request_id)
	{
	...
	bool RtspcStream::ReceiveStop(uint32_t request_id)
	{
		auto packet = ReceivePacket();
		if (packet == nullptr || packet->PayloadLength() <= 0)
		{
	...
	std::shared_ptr<ov::Data> RtspcStream::ReceiveMessage()
	{
	...
	std::shared_ptr<OvtPacket> RtspcStream::ReceivePacket()
	{
