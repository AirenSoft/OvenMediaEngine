...
//
//==============================================================================
#include "decoder_hevc_niquadra.h"
...
bool DecoderHEVCxNIQUADRA::Configure(std::shared_ptr<MediaTrack> context)
{
	...
	try
	{
		...
		pthread_setname_np(_codec_thread.native_handle(), ov::String::FormatString("Dec%sNiquadra", avcodec_get_name(GetCodecID())).CStr());
	}
...
bool DecoderHEVCxNIQUADRA::InitCodec()
{
	const AVCodec *_codec = ::avcodec_find_decoder_by_name("h265_ni_quadra_dec");
	if (_codec == nullptr)
	{
...
void DecoderHEVCxNIQUADRA::CodecThread()
{
	while (!_kill_flag)
	{
