...
#include "codec/decoder/decoder_avc_niquadra.h"
...
#include "codec/decoder/decoder_hevc_niquadra.h"
	...
	for (auto &candidate : *candidates)
	{
		...
		if (candidate->GetCodecId() == cmn::MediaCodecId::H264)
		{
			switch (candidate->GetModuleId())
			{
                ...
                CASE_CREATE_CODEC_IFNEED(NIQUADRA, DecoderAVCxNIQUADRA)
		...
		else if (candidate->GetCodecId() == cmn::MediaCodecId::H265)
		{
			switch (candidate->GetModuleId())
			{
                ...
                CASE_CREATE_CODEC_IFNEED(NIQUADRA, DecoderHEVCxNIQUADRA)
