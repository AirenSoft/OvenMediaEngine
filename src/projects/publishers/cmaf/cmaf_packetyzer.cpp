...
bool CmafPacketyzer::AppendAudioFrame(std::shared_ptr<PacketyzerFrameData> &frame_data)
{
	...
	if (_audio_init == false)
	{
		...
		logtd("CMAF packetizer is initialized for audio for stream [%s/%s]", _app_name.CStr(), _stream_name.CStr());
...
bool CmafPacketyzer::UpdatePlayList()
{
	...
	{
		double availability_time_offset = _media_info.video_framerate != 0 ? (_segment_duration - (1.0 / _media_info.video_framerate)) : _segment_duration;
	...
	if (_audio_sequence_number > 1)
	{
		...
		double availability_time_offset = (_media_info.audio_samplerate / 1024) != 0 ? _segment_duration - (1.0 / ((double)_media_info.audio_samplerate / 1024)) : _segment_duration;
