...
public:
	...
	virtual bool OnPlayListRequest(const std::shared_ptr<http::svr::HttpTransaction> &client,
								   const SegmentStreamRequestInfo &request_info,
								   ov::String &play_list) = 0;

	// Called when the client requests a segment (such as .ts, .m4s)
	virtual bool OnSegmentRequest(const std::shared_ptr<http::svr::HttpTransaction> &client,
								  const SegmentStreamRequestInfo &request_info,
