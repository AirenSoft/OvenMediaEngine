...
class ThumbnailSession : public pub::Session
{
public:
	static std::shared_ptr<ThumbnailSession> Create(const std::shared_ptr<pub::Application> &application,
											  const std::shared_ptr<pub::Stream> &stream,
											  uint32_t ovt_session_id);

	ThumbnailSession(const info::Session &session_info,
			const std::shared_ptr<pub::Application> &application,
			const std::shared_ptr<pub::Stream> &stream);
	~ThumbnailSession() override;
