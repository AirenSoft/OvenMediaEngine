...
#include "thumbnail_export.h"

#include <regex>

std::shared_ptr<ThumbnailSession> ThumbnailSession::Create(const std::shared_ptr<pub::Application> &application,
										  	   const std::shared_ptr<pub::Stream> &stream,
										  	   uint32_t session_id)
{
	auto session_info = info::Session(*std::static_pointer_cast<info::Stream>(stream), session_id);
	auto session = std::make_shared<ThumbnailSession>(session_info, application, stream);
	
	return session;
}

ThumbnailSession::ThumbnailSession(const info::Session &session_info,
...
{
	Stop();
	logtd("ThumbnailSession(%d) has been terminated finally", GetId());
}


bool ThumbnailSession::Start()
{
	logtd("ThumbnailSession(%d) has started.", GetId());
...
bool ThumbnailSession::Stop()
{
	if(_writer != nullptr)
	{
		...
		if( ThumbnailExport::GetInstance()->ExportRecordToXml(GetRecord()->GetFileInfoPath(), GetRecord()) == false )
		{
		...
		logtd("ThumbnailSession(%d) has stoped", GetId());
	}

	return Session::Stop();
}

bool ThumbnailSession::SendOutgoingData(const std::any &packet)
{
...
void ThumbnailSession::OnPacketReceived(const std::shared_ptr<info::Session> &session_info,
									const std::shared_ptr<const ov::Data> &data)
{
	// Not used
}

void ThumbnailSession::SetRecord(std::shared_ptr<info::Record> &record)
{
	_record = record;
}

std::shared_ptr<info::Record>& ThumbnailSession::GetRecord()
{
	return _record;
}

ov::String ThumbnailSession::GetOutputTempFilePath()
{
...
ov::String ThumbnailSession::GetOutputFilePath()
{
	auto app_config = std::static_pointer_cast<info::Application>(GetApplication())->GetConfig();
	auto file_config = app_config.GetPublishers().GetFilePublisher();
...
ov::String ThumbnailSession::GetOutputFileInfoPath()
{
	auto app_config = std::static_pointer_cast<info::Application>(GetApplication())->GetConfig();
	auto file_config = app_config.GetPublishers().GetFilePublisher();
...
bool ThumbnailSession::MakeDirectoryRecursive(std::string s, mode_t mode)
{
	size_t pos = 0;
	std::string dir;
	int32_t mdret;
...
ov::String ThumbnailSession::ConvertMacro(ov::String src)
{
