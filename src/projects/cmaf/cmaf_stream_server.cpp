										 ...
										 const std::shared_ptr<HttpResponse> &response)
{
	...
	{
		//logtd("[%s] Cmaf register chunk response : %s/%s/%s chunk(%u)",
		//	  response->GetRemote()->ToString().CStr(), app_name.CStr(), stream_name.CStr(), file_name.CStr(),
		//	  chunk_item->second->chunked_data->GetLength());
										 ...
										 std::shared_ptr<ov::Data> &chunk_data)
{
	...
	if(chunk_item == _http_chunked_data_list.end())
	{
		...
		_http_chunked_data_list[std::make_tuple(app_name, stream_name, file_name)] = http_chunked_data;
		return;
	}
