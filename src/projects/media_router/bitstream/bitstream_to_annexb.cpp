...
void BitstreamToAnnexB::convert_to(MediaBuffer* pkt)
{
	// 만약, Annex-B 타입의 비트스트림이라면 그냥 종료함.
	if(	pkt->GetByteAt(0) == 0x00 && pkt->GetByteAt(1) == 0x00 && 
		((pkt->GetByteAt(2) == 0x01 ) || (pkt->GetByteAt(2) == 0x00 && pkt->GetByteAt(3) == 0x01))
	)
	{
	...
	uint32_t pbuf_length = pkt->GetBufferSize();
	...
	if(frame_type == 1 && avc_packet_type == 0 )
	{
		...
		pkt->ClearBuffer();
		pkt->AppendBuffer(start_code, 4);
		pkt->AppendBuffer(_sps.data(), _sps.size());
		
		pkt->AppendBuffer(start_code, 4);
		pkt->AppendBuffer(_pps.data(), _pps.size());

		logtd("sps/pps packet size : %d", pkt->GetDataSize());
	}
	// NAL Unit
	/*
	...
	*/
	else if(frame_type == 1 && avc_packet_type == 1 )
	{
		...
		auto tmp_buf = std::make_shared<MediaBuffer>();
		...
		pkt->ClearBuffer();
		pkt->AppendBuffer(tmp_buf->GetBuffer(), tmp_buf->GetBufferSize());
	}
