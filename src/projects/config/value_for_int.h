...
namespace cfg
{
	...
	class Value<Ttype, typename std::enable_if<std::is_same<Ttype, int>::value>::type>
	...
	public:
		...
		{
			*_value = ov::Converter::ToInt32(attribute.value());
			_is_parsed = true;
		...
		{
			*_value = ov::Converter::ToInt32(node.child_value());
			_is_parsed = true;
		...
		{
			return ov::String::FormatString(append_new_line ? "%d\n" : "%d", *_value);
		}

	protected:
		static constexpr ValueType Tconfig_type = ValueType::Integer;
