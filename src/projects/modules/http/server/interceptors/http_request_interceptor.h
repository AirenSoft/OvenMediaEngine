...
namespace http
{
	namespace svr
	{
		...
		public:
			...
			virtual bool IsInterceptorForRequest(const std::shared_ptr<const HttpTransaction> &client) = 0;
			...
			virtual bool OnRequestPrepared(const std::shared_ptr<HttpTransaction> &transaction) = 0;
			...
			virtual ssize_t OnDataReceived(const std::shared_ptr<HttpTransaction> &transaction, const std::shared_ptr<const ov::Data> &data) = 0;
			...
			///
			/// @return Whether to disconnect with the client
			virtual InterceptorResult OnRequestCompleted(const std::shared_ptr<HttpTransaction> &transaction) = 0;
			...
			virtual void OnError(const std::shared_ptr<HttpTransaction> &transaction, StatusCode status_code) = 0;
