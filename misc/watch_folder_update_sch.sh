...
# Check for arguments
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <folder_path_relative_to_root_dir> <name_of_sch_file_to_update>"
    exit 1
fi
...
update_xml() {
    ...
    done < <(find "$FOLDER_PATH" -maxdepth 1 -type f \( -name "*.mp4" -o -name "*.mov" \) -printf "%T+ %p\n" | sort | cut -d " " -f 1-)
...
while inotifywait -e create -e delete -e modify -e move "$FOLDER_PATH"; do
    update_xml
done
